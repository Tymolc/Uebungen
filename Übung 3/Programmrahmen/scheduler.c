...
PDISPATCHER_TASK Schedule(PDISPATCHER_TASK OldThread) {
	...
	static unsigned int StarvationCounter = 0;

	if (!IsListEmpty(&ReadyQueues[SCHED_PRIORITY_HIGH]) && 
				(StarvationCounter<STARVATION_THRESHOLD || IsListEmpty(&ReadyQueues[SCHED_PRIORITY_LOW]))
		)
	{
	...
	printf("Scheduling starving Thread in %d", STARVATION_THRESHOLD-StarvationCounter, " quanta");
